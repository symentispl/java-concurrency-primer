...
https://symentis.pl/en/presentations/[https://symentis.pl/en/presentations/]

**"Java Concurrency Primer"**

you will find there slides and code examples
...
image::https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExb3owNzBzM3dxeHdqYzFlNGk2aXNmYzlzdDU4a3oxMHYyMTZ2cHBsOSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/GD5xkDtFPUpY4/giphy.gif[background]

=== the hardware reward

Sum of `Math.sqrt()` over 100 million doubles — same work, more threads:
...
* The hardware has 8 cores — ignoring them leaves most of the machine idle
* This is free performance; it just requires understanding concurrency

_Run:_ `java code/src/main/java/SpeedupDemo.java`

=== scaling has limits

Past the core count, adding threads stops helping — and eventually hurts:
...
_{asterisk} beyond available core count_

=== physics, stupid!

*Amdahl's Law* — `S(n) = 1 / (1 − p + p/n)` +
Even 5% serial code caps speedup at 20× no matter how many cores you add. +
The serial fraction — startup, coordination, I/O — dominates at scale.

*Universal Scalability Law* (Neil Gunther) — `C(n) = n / (1 + α(n−1) + βn(n−1))` +
Extends Amdahl with a contention penalty α and a coherency penalty β. +
At high thread counts, coordination overhead makes throughput *decrease*. +
This is why every thread pool has a size limit.

[role="highlight_title"]
=== money, money, money

image::https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExMzV3ZnFtdWI2bnBvaHd0NjV2YnQ1Ynp2Nng5OGpkeTRpeDY5YWg4ZCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/JpG2A9P3dPHXaTYrwu/giphy.gif[background]

=== !

software infrastructure costs are exploding, +
it kills projects profitability

**it is estimated to grow 75% this year**

**shortages in hardware supply and increasing costs** +
(geopolitics and AI)

a whole lot of companies is looking for people +
who know how to make software cheaper, scalable and fast

everyone can vibe code microservice and write ecommerce platform +
not many can make it cheap, fast and scalable
...
=== is this safe to call from to threads?

**i++**

=== i++ — two threads, one lost update
...
digraph interleave {
    ...
    A2 [label="Thread A: suspended",                  fillcolor="#4a90d9", fontcolor=white, style="filled,dashed"]
    A3 [label="Thread A: ADD  5+1=6  ← stale read",  fillcolor="#4a90d9", fontcolor=white]
    A4 [label="Thread A: STORE counter=6",            fillcolor="#7a3a3a", fontcolor=white]

    B1 [label="Thread B: (not yet scheduled)",        fillcolor="#c97c2e", fontcolor=white, style="filled,dashed"]
    B2 [label="Thread B: LOAD counter → 5",           fillcolor="#c97c2e", fontcolor=white]
    B3 [label="Thread B: ADD  5+1=6",                 fillcolor="#c97c2e", fontcolor=white]
    B4 [label="Thread B: STORE counter=6",            fillcolor="#c97c2e", fontcolor=white]
    ...
    R [label="counter = 6   expected 7 — one increment lost", fillcolor="#7a3a3a", fontcolor=white, shape=diamond]
    A4 -> R
    B4 -> R
}
----
...
digraph interrupt {
    ...
    subgraph cluster_main {
        ...
        M1 [label="do work", fillcolor="#4a90d9", fontcolor=white]
        M2 [label="worker.interrupt()", fillcolor="#4a90d9", fontcolor=white]
        M3 [label="worker.join()", fillcolor="#5ba05b", fontcolor=white]
        M4 [label="continue", fillcolor="#5ba05b", fontcolor=white]
    ...
    subgraph cluster_worker {
        label="Worker Thread"
        style=filled; fillcolor="#1a3a1a"; color="#5ba05b"; fontcolor=white

        W1 [label="loop: doWork()", fillcolor="#5ba05b", fontcolor=white]
        W2 [label="check interrupted flag", fillcolor="#c97c2e", fontcolor=white, shape=diamond]
        W3 [label="blocking op throws\nInterruptedException", fillcolor="#c97c2e", fontcolor=white]
        W4 [label="restore flag +\nclean up", fillcolor="#c97c2e", fontcolor=white]
        W5 [label="return — thread done", fillcolor="#7a3a3a", fontcolor=white]
    ...
    FLAG [label="interrupted\nflag = true", fillcolor="#6a3a7a", fontcolor=white, shape=ellipse]

    M2 -> FLAG [label="sets"]
    FLAG -> W2  [label="seen on\nnext check", style=dashed]
    FLAG -> W3  [label="wakes\nblocked op", style=dashed]
    M3 -> W5    [style=invis]
}
----
...
_Run:_ `java code/src/main/java/CooperativeShutdown.java`
...
_Run:_ `java code/src/main/java/DataRace.java`
...
A *lock* is the most fundamental synchronization primitive.

* A shared token — only *one* thread can hold it at a time
* *Acquire*: claim the token; if already taken, wait
* *Release*: hand the token back; unblock one waiter
* *Mutual exclusion*: the protected region runs in exactly one thread at a time

Two properties every correct lock must have:

* *Safety* — at most one thread inside the critical section
* *Liveness* — a waiting thread eventually gets in (no deadlock by itself)
...
_Run:_ `java code/src/main/java/SynchronizedCounter.java`
...
_Run:_ `java code/src/main/java/ReadySignal.java`
...
_Run:_ `java code/src/main/java/ProducerConsumer.java`
...
_Run (hangs — kill with Ctrl+C):_ `java code/src/main/java/Deadlock.java`
...
_Run:_ `java code/src/main/java/DeadlockFixed.java`
...
_Run:_ `java code/src/main/java/Livelock.java`
...
_Run:_ `java code/src/main/java/BoundedBufferLock.java`
...
[source,text]
----
"Thread-0" #13 [12345] prio=5 os_prio=0 cpu=1.23ms elapsed=5.32s tid=0x... nid=12345
...
java code/src/main/java/Deadlock.java
...
* `settings=profile` — higher-frequency sampling; lock threshold defaults to *10 ms* (most in-memory locks invisible)
* `code/jfr/contention.jfc` — custom config that lowers locking threshold to *1 ms* for high-frequency contention demos
...
# or start at launch with precompiled classes
java -XX:StartFlightRecording=dumponexit=true,filename=demo.jfr,settings=code/jfr/contention.jfc \
     -cp code/build/classes/java/main ContentionDemo sync
----
...
[source,bash]
----
java -XX:StartFlightRecording=dumponexit=true,filename=ContentionDemo.jfr,settings=code/jfr/contention.jfc \
...
ps -eLo pid,lwp,stat | grep <pid>   # list LWPs with state column (R/S/D/Z)
pidstat -t -p <pid> 1     # per-thread CPU + context-switch rate, 1s interval
----

* Map JVM thread to OS thread: `nid=12345` in thread dump matches `TID 12345` in `top -H` directly (JDK 17+: nid is already decimal)
* High context-switch rate with low CPU → threads fighting over a lock or spinning

== what's next?

* Learn java.util.concurrent 
* "Java Concurrency in Practice" by Brain Goetz
* "Seven concurrency models in seven weeks" by Paul Butcher
* "Grokking concurrency" by Kirill Bobrov

if you read those and don't have enough

=== going even deeper

* Java memory model, all presentations by Aleksey Shipilëv about Java internals
* C++ memory model (holds true for Java) by Herb Sutter and Fedor G. Pikus
